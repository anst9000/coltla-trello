...
export async function POST(req: Request) {
  // Cuando se realiza el pago stripe lanza una solicitud a este endpoint
  ...
  try {
    event = stripe.webhooks.constructEvent(
      // Se utiliza stripe.webhooks.constructEvent para validar la firma de la solicitud
  ...
  if (event.type === "checkout.session.completed") {
    // Si el evento fue un pago exitoso (creación por 1ª vez de una suscripción)
    const subscription = await stripe.subscriptions.retrieve(
      // se  recupera información detallada sobre la suscripción asociada a la sesión de pago.
    ...
    if (!session?.metadata?.orgId) {
      // Se verifica si el campo orgId está presente en los metadatos de la sesión de pago.
  ...
  if (event.type === "invoice.payment_succeeded") {
    // Si el evento es el pago de una factura con exito (renovación de la suscripción)

    const subscription = await stripe.subscriptions.retrieve(
      // volvemos a recuperar información detallada sobre la suscripción asociada a la sesión de pago
      session.subscription as string
    )

    await db.orgSubscription.update({
      // y se actualiza un registro existente en la tabla orgSubscription en la base de datos.
      where: {
