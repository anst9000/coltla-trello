...
const DAY_IN_MS = 24 * 60 * 60 * 1000 // Cantidad de milisegundos en un día (24 horas).
...
export const checkSubscription = async () => {
  ...
  const orgSubscription = await db.orgSubscription.findUnique({
    // Busca una suscripción de la organización en la base de datos(db.orgSubscription)
    where: {
      // basada en orgId.
      orgId,
    },
    select: {
      // Selecciona los campos que se desean mostrar
  ...
  if (!orgSubscription) {
    // Si no hay subscripción devuelve false.
    return false
  }
